...
test.describe('S3 compatibility endpoints', () => {
  ...
  test('SD compartment prefix can Put/Get/List/Delete objects', async () => {
    ...
    const sdHead = await client.send(
      new HeadObjectCommand({
        Bucket: bucketName,
        Key: sdKey,
      }),
    ...
    const sdObject = await client.send(
      new GetObjectCommand({
        Bucket: bucketName,
        Key: sdKey,
      }),
    ...
    expect(sdContent ?? '').toContain(
      'Dummy payload for S3 compatibility tests.',
    )
  ...
  test('Multipart lifecycle works', async () => {
    test.skip(
      process.env.S3_TEST_ENABLE_MULTIPART !== 'true',
      'Set S3_TEST_ENABLE_MULTIPART=true to run multipart checks',
    )
    const client = createS3Client()
    ...
    try {
      const partOneResult = await client.send(
      ...
      const partTwoResult = await client.send(
        new UploadPartCommand({
          Bucket: bucketName,
          Key: multipartKey,
          UploadId: uploadId,
