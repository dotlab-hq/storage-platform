...
const AUTH_HANDLER_TIMEOUT_MS = Number(
  process.env.AUTH_HANDLER_TIMEOUT_MS ?? 10000,
)
  ...
  try {
    ...
    const response = await Promise.race([
      ...
      new Promise<Response>((resolve) => {
        ...
        setTimeout(
          () => resolve(createServiceUnavailableResponse(timeoutMessage)),
    ...
    const fallbackMessage =
      'Authentication request failed due to database connectivity. Please retry shortly.'
