...
export const Route = createFileRoute( "/api/storage/s3/complete-upload" as never )( {
    component: () => null,
    server: {
        handlers: {
            POST: async ( { request } ) => {
                try {
                    ...
                    const file = await completeUpload( currentUser.id, payload.uploadId, payload.etag )
