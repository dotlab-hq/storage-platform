...
const BucketCorsRuleLowerSchema = z.object({
...
const BucketCorsRulePascalSchema = z.object({
  AllowedOrigins: z.array(z.string()),
  AllowedMethods: z.array(z.string()),
  AllowedHeaders: z.array(z.string()),
  ExposeHeaders: z.array(z.string()),
...
const BucketCorsUpdateSchema = z
  .object({
    action: z.literal('cors'),
    bucketName: z.string().min(3),
    rules: z
      .array(z.union([BucketCorsRuleLowerSchema, BucketCorsRulePascalSchema]))
      .optional(),
    CORSRules: z.array(BucketCorsRulePascalSchema).optional(),
  })
  .refine(
    (value) => value.rules !== undefined || value.CORSRules !== undefined,
    {
      message: 'rules or CORSRules is required',
      path: ['rules'],
    },
  )
...
const UpdateBucketSchema = z.discriminatedUnion('action', [
  z.object({
    action: z.literal('versioning'),
    bucketName: z.string().min(3),
    state: z.enum(['enabled', 'suspended', 'disabled']),
  }),
  z.object({
    ...
    handlers: {
      GET: async ({ request }) => {
        try {
          ...
          return Response.json(
            { error: toError(error, 'Failed to load bucket settings') },
            { status: 500 },
          )
        }
      },
      PUT: async ({ request }) => {
        try {
          ...
          if (payload.action === 'cors') {
            const sourceRules = payload.rules ?? payload.CORSRules ?? []
            ...
            await replaceBucketCors(bucket.id, normalizedRules)
          }
          ...
          return Response.json(
            {
              ok: false,
              error: toError(error, 'Failed to update bucket settings'),
            },
            { status: 500 },
          )
        }
