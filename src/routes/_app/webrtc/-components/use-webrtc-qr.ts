...
export function useWebrtcQr( isConnected: boolean ) {
    ...
    const generateQrQuery = useQuery({
        ...
        queryFn: async () => {
            const response = await fetch( '/api/qr-auth/create-offer', { method: 'POST' } )
            const data = ( await response.json() )
            if ( !response.ok || !( 'payload' in ( data as any ) ) ) {
                throw new Error( ( data as { error?: string } ).error ?? 'Failed to generate QR offer.' )
            }
            const dataUrl = await QRCode.toDataURL( ( data as any ).payload, { width: 260, margin: 1 } )
    ...
    React.useEffect(() => {
        if (generateQrQuery.error) {
            setErrorMessage(generateQrQuery.error instanceof Error ? generateQrQuery.error.message : 'Failed to generate QR code.')
            setExpired(true)
            setOffer(null)
            setQrImage('')
        }
        ...
        const poll = async () => {
            ...
            try {
                ...
                if ( status === 'approved' || status === 'claimed' ) {
                    const pollInterval = ( pollData as { pollIntervalMs?: number } ).pollIntervalMs || 1000
                    if ( !cancelled ) {
