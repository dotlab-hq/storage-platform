...
export function HeaderUploadMenu( {
    ...
    const uploadFiles = createClientOnlyFn( async ( files: File[], currentUserId: string ) => {
        const errors: string[] = []
    ...
    const resolveUserId = createClientOnlyFn( async ( incomingUserId: string | null ) => {
        if ( incomingUserId ) {
        ...
        <>
            ...
            <Dialog open={open} onOpenChange={setOpen}>
                <DialogContent>
                    ...
                    <div
                        className={`rounded-lg border border-dashed p-6 text-center ${isDragging ? "bg-muted" : ""
                            }`}
                    ...
                    <div className="max-h-56 overflow-y-auto rounded-md border">
                        {selectedFiles.length === 0 ? (
                            <p className="text-muted-foreground p-3 text-sm">No files selected.</p>
                        ) : (
