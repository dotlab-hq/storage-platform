...
type SearchResult = {
    folders?: { id: string; name: string; createdAt: string; parentFolderId: string | null }[]
    files?: { id: string; name: string; sizeInBytes: number; mimeType?: string | null; objectKey?: string; createdAt: string }[]
    error?: string
}
...
export function TopbarActions( {
    ...
    const handleSearchChange = React.useCallback(
        ( value: string ) => {
            ...
            if ( !value.trim() ) { onSearch?.( null ); return }
            debounceRef.current = setTimeout( () => {
                if ( !userId ) return
                void searchOnClient( userId, value.trim() ).then( ( data ) => {
                    const items: StorageItem[] = [
                        ...( data.folders ?? [] ).map( ( f ) => ( {
                            ...f, type: "folder" as const, userId,
                            parentFolderId: f.parentFolderId ?? null,
                            createdAt: new Date( f.createdAt ), updatedAt: new Date( f.createdAt ),
                        } ) ),
                        ...( data.files ?? [] ).map( ( f ) => ( {
                            ...f, type: "file" as const, userId,
                            objectKey: f.objectKey ?? "", mimeType: f.mimeType ?? null,
                            folderId: null,
                            createdAt: new Date( f.createdAt ), updatedAt: new Date( f.createdAt ),
                        } ) ),
                    ]
