...
function clampToViewport(x: number, y: number) {
  ...
  const maxY = Math.max(
    VIEWPORT_MARGIN_PX,
    window.innerHeight - MENU_HEIGHT_PX - VIEWPORT_MARGIN_PX,
  )
  return {
...
export function GlobalShellActions({
  children,
}: {
  ...
  return (
    <>
      ...
      {contextOpen && (
        ...
        >
          {activeConfig.contextActions.length > 0 ? (
            activeConfig.contextActions.map((action) => (
              <button
                key={action.id}
                type="button"
                className={cn(
