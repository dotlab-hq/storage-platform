...
export function ProviderContentsModal( {
    ...
    useEffect( () => {
        maybeLoadMore()
    }, [viewer.files.length, viewer.folders.length, viewer.hasNextPage, viewer.isFetchingNextPage] )
                ...
                <DialogHeader className="border-b bg-linear-to-br from-background via-background to-muted/20 px-6 py-5">
                    ...
                    <div className="mt-4 grid gap-3 sm:grid-cols-3">
                        ...
                        <div className="rounded-xl border bg-background/80 p-3">
                            ...
                            <p className="truncate text-sm font-medium">{provider?.bucketName ?? '-'}</p>
                        </div>
                        <div className="rounded-xl border bg-background/80 p-3">
                            <p className="text-[11px] uppercase tracking-wide text-muted-foreground">
                                Prefix
                    ...
                    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                        <div className="flex flex-wrap items-center gap-1 text-sm text-muted-foreground">
                            {breadcrumbNodes.map( ( crumb, index ) => (
                                <div key={`${crumb.label}-${crumb.value}`} className="flex items-center gap-1">
