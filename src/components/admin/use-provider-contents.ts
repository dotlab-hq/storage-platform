...
async function loadProviderContents( {
    ...
    const response = await fetch(
        `/api/admin/storage-providers/${encodeURIComponent( providerId )}/contents${searchParams.toString().length > 0 ? `?${searchParams.toString()}` : ''
        }`,
        {
            credentials: 'include',
        },
...
export function useProviderContents( providerId: string | null, open: boolean ) {
    ...
    const query = useInfiniteQuery( {
        queryKey: ['admin-provider-contents', providerId, prefix],
