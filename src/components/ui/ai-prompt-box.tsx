...
>((props, ref) => {
  ...
  // Sync external value to internal state when controlled
  React.useEffect(() => {
    if (isControlled) {
  ...
  const handleSubmit = () => {
    if (effectiveValue.trim() || files.length > 0) {
      let messagePrefix = ''
      if (showSearch) messagePrefix = '[Search: '
      else if (showThink) messagePrefix = '[Think: '
      else if (showCanvas) messagePrefix = '[Canvas: '
      const formattedInput = messagePrefix
        ? `${messagePrefix}${effectiveValue}]`
        : effectiveValue
      onSend(formattedInput, files)
      if (!isControlled) {
        setInternalValue('')
      }
      onValueChange?.('') // notify parent to clear if controlled
      setFiles([])
      setFilePreviews({})
    }
  }
  ...
  const hasContent = effectiveValue.trim() !== '' || files.length > 0

  // Button click handler with stop support
  ...
  const buttonDisabled = isLoading && !hasContent && !onStop
  ...
  return (
    <>
      <PromptInput
        ...
        className={cn(
          'w-full bg-background border-border shadow-[0_8px_30px_rgba(0,0,0,0.24)] transition-all duration-300 ease-in-out',
      ...
      >
        ...
        <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
          ...
          >
            ...
            >
              {isLoading ? (
                <Square className="h-4 w-4 fill-foreground animate-pulse" />
              ) : isRecording ? (
