...
export function TextFileEditorDialog({
  ...
  const { isSaving, handleSave } = useEditorSave({
    quillRef, userId, item, fileName, currentFolderId, onSaved, onOpenChange
  })
  ...
  React.useEffect(() => {
    const handleMediaInputChange = async (event: Event) => {
      ...
      try {
        const uploaded = await uploadFileToS3(file, userId, currentFolderId, () => undefined)
        const mediaUrl = buildFileRedirectUrl({ folderId: currentFolderId, fileId: uploaded.id })
        ...
        toast.error(error instanceof Error ? error.message : `Failed to upload ${expectedKind}`)
      }
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
            ...
            <div className="min-w-0 flex-1">
              ...
              {initError ? (
                ...
                <p className="truncate text-xs text-muted-foreground">
                  {item?.type === 'file' ? 'Editing existing file' : 'Creating new file'}
                </p>
              )}
            </div>
          </div>
          <div className="flex shrink-0 items-center gap-2 pl-4">
            <Button variant="ghost" size="sm" onClick={handleClose} disabled={isSaving || isLoading}>
              <X className="h-4 w-4" />
            ...
            >
              ...
              <span className="sr-only md:not-sr-only">{isSaving ? 'Saving...' : 'Save File'}</span>
            </Button>
          </div>
        </div>
        <div className="relative flex-1 bg-muted/20">
          ...
          <div
            ref={editorRef}
            className={cn('h-full bg-background [&_.ql-container]:text-base [&_.ql-editor]:min-h-[calc(85vh-8rem)] [&_.ql-editor]:pb-16 [&_.ql-toolbar]:sticky [&_.ql-toolbar]:top-0 [&_.ql-toolbar]:z-10 [&_.ql-toolbar]:bg-background [&_.ql-toolbar]:border-x-0 [&_.ql-toolbar]:border-t-0', !isEditorReady && 'invisible')}
          />
        </div>
      </DialogContent>
      <input type="file" accept="image/*,video/*" ref={mediaInputRef} className="hidden" />
    </Dialog>
  )
}
