...
export function MoveModal( {
    ...
    const allPaths = useMemo( () => buildFolderPathOptions( allFolders ), [allFolders] )

    const movingFolderIds = useMemo( () =>
        items.filter( ( item ) => item.type === "folder" ).map( ( folder ) => folder.id ), [items]
    )

    const availableFolders = useMemo( () => allPaths.filter( ( folder ) => {
    ...
    const filteredFolders = useMemo( () => {
        ...
        return availableFolders.filter( ( folder ) =>
            folder.path.toLowerCase().includes( query ) || folder.name.toLowerCase().includes( query )
        )
    ...
    const selectedPath = targetId
        ? availableFolders.find( ( folder ) => folder.id === targetId )?.path ?? "Unknown path"
        : "/"
            ...
            <DialogContent className="overflow-hidden sm:max-w-2xl">
                ...
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2">
                        ...
                        {isUpdatePath ? `Update path for ${itemLabel}` : `Move ${itemLabel}`}
                    </DialogTitle>
                    <DialogDescription>
                        Pick a destination path directly. No manual folder-by-folder navigation required.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div className="bg-muted/40 rounded-md border px-3 py-2 text-xs">
                    Destination: <span className="text-foreground font-medium">{hasSelection ? selectedPath : "No destination selected"}</span>
                </div>
                <div className="max-h-72 space-y-1 overflow-y-auto rounded-md border p-2">
                    {fetching ? (
                        <p className="text-muted-foreground animate-pulse px-3 py-4 text-center text-sm">Loading folders...</p>
                    ) : (
                        <>
                            <button
                                type="button"
