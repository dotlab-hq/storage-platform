...
type BucketSettingsPayload = {
    bucket: { name: string, region: string, objectOwnershipMode: string, blockPublicAccess: boolean }
    versioning: "enabled" | "suspended" | "disabled"
    policyJson: string
    corsRules: Array<{ allowedOrigins: string[], allowedMethods: string[], allowedHeaders: string[], exposeHeaders: string[], maxAgeSeconds: number | null }>
    acl: "private" | "public-read"
}

async function fetchSettings( bucketName: string ): Promise<BucketSettingsPayload> {
    const response = await fetch( `/api/storage/s3/bucket-settings?bucketName=${encodeURIComponent( bucketName )}` )
...
export function BucketSettingsDialog( props: BucketSettingsDialogProps ) {
    ...
    const mutation = useMutation( {
        ...
        onMutate: async ( payload ) => {
            await queryClient.cancelQueries( { queryKey: ["bucket-settings", bucketName] } )
            const previous = queryClient.getQueryData<BucketSettingsPayload>( ["bucket-settings", bucketName] )
            ...
            if ( actionPayload.action === "versioning" && "state" in actionPayload ) {
                queryClient.setQueryData<BucketSettingsPayload>( ["bucket-settings", bucketName], { ...previous, versioning: actionPayload.state as BucketSettingsPayload["versioning"] } )
            }
            if ( actionPayload.action === "acl" && "cannedAcl" in actionPayload ) {
                queryClient.setQueryData<BucketSettingsPayload>( ["bucket-settings", bucketName], { ...previous, acl: actionPayload.cannedAcl as BucketSettingsPayload["acl"] } )
            }
            if ( actionPayload.action === "policy" && "policyJson" in actionPayload ) {
                queryClient.setQueryData<BucketSettingsPayload>( ["bucket-settings", bucketName], { ...previous, policyJson: actionPayload.policyJson as string } )
            }
            if ( actionPayload.action === "cors" && "rules" in actionPayload ) {
                queryClient.setQueryData<BucketSettingsPayload>( ["bucket-settings", bucketName], { ...previous, corsRules: actionPayload.rules as BucketSettingsPayload["corsRules"] } )
            }
        ...
        onError: ( _error, _payload, context ) => {
            if ( context?.previous ) {
                queryClient.setQueryData( ["bucket-settings", bucketName], context.previous )
            }
        },
        onSuccess: async () => {
            await queryClient.invalidateQueries( { queryKey: ["bucket-settings", bucketName] } )
        },
    } )
    ...
    const corsPretty = useMemo( () => JSON.stringify( payload?.corsRules ?? [], null, 2 ), [payload?.corsRules] )
        ...
        <Dialog open={bucketName !== null} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl">
                <DialogHeader>
                    ...
                    <DialogDescription>{bucketName ? `Configure ${bucketName}` : ""}</DialogDescription>
                </DialogHeader>
                ...
                {mutation.error && <div className="rounded border border-destructive/40 bg-destructive/10 p-2 text-xs text-destructive">{mutation.error.message}</div>}

                {payload && activeTab === "overview" && (
                    <div className="space-y-2 text-sm">
                        <p>Region: {payload.bucket.region}</p>
                        <p>Ownership: {payload.bucket.objectOwnershipMode}</p>
                        <p>Block Public Access: {payload.bucket.blockPublicAccess ? "Enabled" : "Disabled"}</p>
                    </div>
                )}

                {payload && activeTab === "permissions" && (
                    <div className="space-y-3">
                        <textarea aria-label="Bucket policy JSON" className="h-40 w-full rounded border p-2 text-xs" value={draftPolicy || payload.policyJson} onChange={( e ) => setDraftPolicy( e.target.value )} />
                        <div className="flex items-center gap-2">
                            <Button size="sm" onClick={() => mutation.mutate( { action: "policy", bucketName, policyJson: draftPolicy || payload.policyJson } )}>
                                <Save className="h-4 w-4" /> Save Policy
                            </Button>
                            <Button size="sm" variant="outline" onClick={() => mutation.mutate( { action: "acl", bucketName, cannedAcl: payload.acl === "private" ? "public-read" : "private" } )}>
                ...
                {payload && activeTab === "versioning" && (
                    <div className="flex items-center gap-2">
                        ...
                        <Button size="sm" onClick={() => mutation.mutate( { action: "versioning", bucketName, state: "enabled" } )}>Enable</Button>
                        <Button size="sm" variant="outline" onClick={() => mutation.mutate( { action: "versioning", bucketName, state: "suspended" } )}>Suspend</Button>
                        <Button size="sm" variant="outline" onClick={() => mutation.mutate( { action: "versioning", bucketName, state: "disabled" } )}>Disable</Button>
                    </div>
                )}
            </DialogContent>
        </Dialog>
