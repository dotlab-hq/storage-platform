...
type ShareResponse = { link?: ShareLinkInfo | null; error?: string }

const fetchShareLink = createClientOnlyFn( async ( userId: string, itemId: string, itemType: "file" | "folder" ) => {
    const params = new URLSearchParams( { userId, itemId, itemType } )
    const res = await fetch( `/api/storage/share?${params}` )
    return ( await res.json() ) as ShareResponse
} )

const postShareAction = createClientOnlyFn( async ( body: Record<string, unknown> ) => {
...
export function ShareModal( { open, onOpenChange, item, userId }: ShareModalProps ) {
    ...
    const loadLink = useCallback( async () => {
        ...
        setLoading( true )
        try {
            const data = await fetchShareLink( userId, item.id, item.type )
    ...
    const handleToggleOff = async () => {
        ...
        setLoading( true )
        try {
            await postShareAction( { action: "toggle", userId, linkId: link.id, isActive: false } )
            setLink( null )
            toast.success( "Share link disabled" )
        } finally { setLoading( false ) }
    }

    const shareUrl = link ? `${typeof window !== "undefined" ? window.location.origin : ""}/share/${link.shareToken}` : ""
...
function ActiveShareView( { shareUrl, onCopy, onDisable }: {
    shareUrl: string; onCopy: () => void; onDisable: () => void
} ) {
    return (
