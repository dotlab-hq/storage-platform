  ...
  if ( !attemptedDelete ) {
    try {
      ...
      if ( versionRows.length === 0 ) {
        console.log( '[Deletion] No alternate keys found for file, skipping S3 delete' )
      } else {
        for ( const vr of versionRows ) {
          ...
          for ( const key of keysToTry ) {
            try {
              ...
              console.log( '[Deletion] Attempting fallback S3 delete for key:', key )
              await provider.client.send(
                new DeleteObjectCommand( { Bucket: provider.bucketName, Key: key } ),
              )
              console.log( '[Deletion] Fallback S3 delete successful for key:', key )
            } catch ( err ) {
              console.warn( '[Deletion] Fallback S3 delete failed for key:', key, err )
      ...
      console.error( '[Deletion] Error while attempting fallback key deletion:', err )
