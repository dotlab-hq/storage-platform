...
const CorsRuleSchema = z.object( {
    AllowedOrigins: z.array( z.string().min( 1 ) ).min( 1, 'At least one origin is required' ),
    AllowedMethods: z.array(
        z.enum( ['GET', 'PUT', 'POST', 'DELETE', 'HEAD', 'OPTIONS'] ),
...
export const CorsConfigSchema = z.object( {
    CORSRules: z.array( CorsRuleSchema ).min( 1, 'At least one CORS rule is required' ),
} )

export const CorsConfigLowercaseSchema = z.object( {
    CORSRules: z.array( CorsRuleLowercaseSchema ).min( 1, 'At least one CORS rule is required' ),
} )
 ...
 */
export function validateCorsConfig( config: unknown ): { valid: boolean; errors?: Record<string, string[]> } {
 ...
 */
export function convertDatabaseToCorsConfig( rules: Array<{
    ...
    return {
        CORSRules: rules.map( ( rule ) => ( {
            ...
            AllowedHeaders: rule.allowedHeadersJson ? JSON.parse( rule.allowedHeadersJson ) : ['*'],
            ExposeHeaders: rule.exposeHeadersJson ? JSON.parse( rule.exposeHeadersJson ) : [],
