...
export const createS3ViewerFolderFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    const { createS3ViewerFolderFn: serverFn } = await import(
      '@/lib/storage/mutations/s3-viewer/viewer-fns.server'
    )
...
export const deleteS3ViewerObjectFn = createServerFn({ method: 'POST' })
  .inputValidator(ObjectKeySchema)
  .handler(async ({ data }) => {
    const { deleteS3ViewerObjectFn: serverFn } = await import(
      '@/lib/storage/mutations/s3-viewer/viewer-fns.server'
    )
    return serverFn({ data })
  })
...
export const createS3ViewerPresignUrlFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    const { createS3ViewerPresignUrlFn: serverFn } = await import(
      '@/lib/storage/mutations/s3-viewer/viewer-fns.server'
    )
    return serverFn({ data })
  })
