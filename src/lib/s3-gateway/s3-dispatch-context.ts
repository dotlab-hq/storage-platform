...
export function mapProviderStatusToS3Error(statusCode: number): {
  status: number
  code: string
  message: string
} {
  if (statusCode === 400)
    return {
      status: 400,
      code: 'InvalidRequest',
      message: 'The request sent to the upstream storage provider was invalid',
    }
  if (statusCode === 401 || statusCode === 403)
    return {
      status: 403,
      code: 'AccessDenied',
      message: 'Access denied by upstream storage provider',
    }
  if (statusCode === 404)
    return {
      status: 404,
  ...
  return {
    status: 502,
    code: 'BadGateway',
    message: 'Upstream storage provider request failed',
  }
}
...
export async function resolveAuthorizedBucket(
  request: Request,
...
export async function ensureAccess(
  ...
  const signedIdentity = accessKeyId
    ? isSigV4Valid(request, bucket) ||
      (request.headers.get('x-s3-secret-access-key')
        ? isSecretValid(
            bucket,
            request.headers.get('x-s3-secret-access-key') ?? '',
          )
