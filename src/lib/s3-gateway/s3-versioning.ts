...
export async function getBucketVersioningState( bucketId: string ): Promise<string> {
    const rows = await db.select( { state: virtualBucket.versioningState } ).from( virtualBucket ).where( eq( virtualBucket.id, bucketId ) ).limit( 1 )
    return rows[0]?.state ?? "disabled"
}

export async function createObjectVersionFromCurrent( bucket: BucketContext, objectKey: string, versionId?: string ): Promise<string | null> {
    ...
    const rows = await db
        ...
        .from( file )
        .where( and( eq( file.userId, bucket.userId ), eq( file.objectKey, upstreamObjectKey ), eq( file.isDeleted, false ) ) )
        .limit( 1 )
    ...
    await provider.client.send( new CopyObjectCommand( {
...
export async function createDeleteMarkerVersion( bucket: BucketContext, objectKey: string ): Promise<string> {
...
export async function getObjectVersionResponse( bucket: BucketContext, objectKey: string, versionId: string, headOnly: boolean ): Promise<Response | null> {
    const rows = await db
        .select( {
        ...
        .from( fileVersion )
        .where( and( eq( fileVersion.bucketId, bucket.bucketId ), eq( fileVersion.objectKey, objectKey ), eq( fileVersion.versionId, versionId ) ) )
        .limit( 1 )
    ...
    const command = headOnly
        ? new HeadObjectCommand( { Bucket: provider.bucketName, Key: version.upstreamObjectKey } )
        : new GetObjectCommand( { Bucket: provider.bucketName, Key: version.upstreamObjectKey } )
...
export async function listObjectVersions( bucketId: string ): Promise<Array<{ key: string, versionId: string, isDeleteMarker: boolean, lastModified: Date, etag: string | null, size: number }>> {
    const rows = await db
        .select( {
...
export async function restoreObjectVersion( bucket: BucketContext, objectKey: string, versionId: string ): Promise<void> {
    const rows = await db
        .select( {
            upstreamObjectKey: fileVersion.upstreamObjectKey,
        ...
        .from( fileVersion )
        .where( and( eq( fileVersion.bucketId, bucket.bucketId ), eq( fileVersion.objectKey, objectKey ), eq( fileVersion.versionId, versionId ), eq( fileVersion.isDeleteMarker, false ) ) )
        .limit( 1 )
    ...
    const currentKey = buildUpstreamObjectKey( bucket.userId, bucket.bucketId, bucket.mappedFolderId, objectKey )
    await provider.client.send( new CopyObjectCommand( {
        Bucket: provider.bucketName,
        Key: currentKey,
    ...
    await db.update( file ).set( { isDeleted: false, deletedAt: null } ).where( and( eq( file.userId, bucket.userId ), eq( file.objectKey, currentKey ) ) )
}
