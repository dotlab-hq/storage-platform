...
export function parseCompleteMultipartUploadParts( xml: string ): CompletedMultipartPart[] {
    ...
    for ( const match of partMatches ) {
        ...
        if ( !partNumberMatch || !eTagMatch ) {
            throw new Error( "MalformedXML: each Part must include ETag and PartNumber" )
        }
