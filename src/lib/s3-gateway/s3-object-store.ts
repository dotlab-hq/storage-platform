...
/**
 * Convert a ReadableStream to a Node.js Readable for AWS SDK compatibility.
 */
function toNodeReadable( stream: ReadableStream<Uint8Array> ): Readable {
  const reader = stream.getReader()
...
export async function getObject(
  ...
  if ( stored.sizeInBytes > PROXY_STREAMING_THRESHOLD && provider.proxyUploadsEnabled ) {
    // Use proxy streaming for large files to handle GB-sized movies
    return streamLargeObjectViaProxy( bucket, stored, provider, effectiveLastModified )
  }
