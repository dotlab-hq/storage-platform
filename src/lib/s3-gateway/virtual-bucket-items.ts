...
export async function listVirtualBucketItems( userId: string, bucketName: string ) {
    ...
    const filesInBucket = await db
        ...
        .from( file )
        .where( and( eq( file.userId, userId ), eq( file.folderId, mappedFolderId ), eq( file.isDeleted, false ) ) )
