...
export async function evaluateBucketPolicy(
  ...
  for (const statement of statements) {
    ...
    const actionOk = toArray(statement.Action).some((candidate) =>
      actionMatches(action, candidate),
    )
    if (!actionOk) {
      continue
    }
    const resourceOk = toArray(statement.Resource).some((candidate) =>
      resourceMatches(resource, candidate),
    )
    if (!resourceOk) {
      continue
    }
