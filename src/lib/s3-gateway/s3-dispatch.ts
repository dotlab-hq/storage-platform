import { parseAccessKeyId, resolveBucketByName } from "@/lib/s3-gateway/s3-context"
...
import { mapProviderStatusToS3Error, providerHttpStatusCode } from "@/lib/s3-gateway/s3-dispatch-context"
import { handleGet, handleHead } from "@/lib/s3-gateway/s3-dispatch-read"
import { handlePut } from "@/lib/s3-gateway/s3-dispatch-put"
import { handleDelete, handlePost } from "@/lib/s3-gateway/s3-dispatch-mutations"
...
export async function handleS3Request( request: Request ): Promise<Response> {
    ...
    const parsedPath = parseS3Path( request.url )
    const bucketForCors = parsedPath.bucketName ? await resolveBucketByName( parsedPath.bucketName ) : null
    ...
    try {
        ...
        const corsResponse = await applyBucketCors( request, await response, bucketForCors )
        ...
        const message = error instanceof Error ? `${error.name}: ${error.message}` : "Unknown server error"
        ...
        if ( error instanceof ProviderRequestTimeoutError ) {
            ...
            if ( upstreamStatusCode !== null ) {
                ...
                response = s3ErrorResponse( mapped.status, mapped.code, mapped.message, resource )
