...
import { isBucketPublicReadable, isObjectPublicReadable } from "@/lib/s3-gateway/s3-acl"

export type S3Action = "s3:ListBucket" | "s3:GetObject" | "s3:PutObject" | "s3:DeleteObject" | "s3:GetBucketAcl" | "s3:PutBucketAcl" | "s3:GetBucketPolicy" | "s3:PutBucketPolicy"
...
export async function isActionAllowed( input: {
    ...
    const resource = input.objectKey ? objectArn( input.bucket.bucketName, input.objectKey ) : bucketArn( input.bucket.bucketName )

    const policyDecision = await evaluateBucketPolicy( input.bucket.bucketId, input.accessKeyId, input.action, resource )
    ...
    if ( input.action === "s3:GetObject" && input.objectKey ) {
        const objectPublic = await isObjectPublicReadable( input.bucket, input.objectKey )
        if ( objectPublic ) {
            return true
        }
        return isBucketPublicReadable( input.bucket.bucketId )
