  ...
  const ruleXml = rules
    .map((rule) => {
      const origins = rule.allowedOrigins
        .map((origin) => tag('AllowedOrigin', origin))
        .join('')
      const methods = rule.allowedMethods
        .map((method) => tag('AllowedMethod', method))
        .join('')
      const headers = rule.allowedHeaders
        .map((header) => tag('AllowedHeader', header))
        .join('')
      const exposed = rule.exposeHeaders
        .map((header) => tag('ExposeHeader', header))
        .join('')
      const maxAge =
        rule.maxAgeSeconds === null
          ? ''
          : tag('MaxAgeSeconds', rule.maxAgeSeconds)
...
export function listMultipartUploadsXml(
  ...
  const uploadsXml = uploads
    .map(
      (upload) =>
        `<Upload>${tag('Key', upload.key)}${tag('UploadId', upload.uploadId)}${tag('Initiated', upload.initiated.toISOString())}</Upload>`,
    )
    .join('')
...
export function listPartsXml(
  ...
  objectKey: string,
  uploadId: string,
  parts: MultipartPartListItem[],
): string {
  const partsXml = parts
    .map(
      (part) =>
        `<Part>${tag('PartNumber', part.partNumber)}${tag('LastModified', part.lastModified.toISOString())}${tag('ETag', part.etag ?? '')}${tag('Size', part.size)}</Part>`,
    )
    .join('')
...
export function parseBucketCorsXml(xml: string): BucketCorsRuleRecord[] {
  ...
  return ruleMatches.map((match) => {
    const block = match[1]
    const maxAgeMatch = block.match(
      /<MaxAgeSeconds>\s*(\d+)\s*<\/MaxAgeSeconds>/i,
    )
    return {
