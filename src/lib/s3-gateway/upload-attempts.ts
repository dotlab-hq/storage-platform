...
export async function completeUpload(
  ...
  const attemptRows = await db
    ...
    .where(
      and(eq(uploadAttempt.id, uploadId), eq(uploadAttempt.userId, userId)),
    )
    .limit(1)
  ...
  if (attempt.expiresAt.getTime() < Date.now()) {
    await db
      ...
      .set({ status: 'failed', errorMessage: 'Upload expired' })
  ...
  if (observedSize !== attempt.expectedSize) {
    await db
      .update(uploadAttempt)
      .set({ status: 'failed', errorMessage: 'Uploaded size mismatch' })
      .where(eq(uploadAttempt.id, uploadId))
