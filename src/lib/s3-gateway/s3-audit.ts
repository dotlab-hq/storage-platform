...
export function inferS3Operation( request: Request ): string {
    ...
    if ( request.method === "GET" && url.searchParams.has( "versioning" ) ) return "GetBucketVersioning"
    if ( request.method === "PUT" && url.searchParams.has( "versioning" ) ) return "PutBucketVersioning"
    if ( request.method === "GET" && url.searchParams.has( "versions" ) ) return "ListObjectVersions"
    if ( request.method === "GET" && url.searchParams.has( "tagging" ) ) return "GetObjectTagging"
    if ( request.method === "PUT" && url.searchParams.has( "tagging" ) ) return "PutObjectTagging"
    if ( request.method === "DELETE" && url.searchParams.has( "tagging" ) ) return "DeleteObjectTagging"
