...
function headersMatch( allowedHeaders: string[], requestedHeaders: string[] ): boolean {
    if ( allowedHeaders.length === 0 || allowedHeaders.includes( "*" ) ) return true
    return requestedHeaders.every( ( header ) => includesCaseInsensitive( allowedHeaders, header ) )
}

export async function applyBucketCors( request: Request, response: Response, bucket: BucketContext | null ): Promise<Response> {
    ...
    const matched = rules.find( ( rule ) => {
        return originMatches( rule.allowedOrigins, origin )
