...
import { multipartUploadPart, uploadAttempt, virtualBucket } from "@/db/schema/s3-gateway"
...
export async function getBucketLocation( bucketId: string ): Promise<string> {
    const rows = await db.select( { region: virtualBucket.region } ).from( virtualBucket ).where( eq( virtualBucket.id, bucketId ) ).limit( 1 )
...
export async function setBucketVersioning( bucketId: string, versioningState: string ): Promise<void> {
    await db.update( virtualBucket ).set( { versioningState } ).where( eq( virtualBucket.id, bucketId ) )
}

export async function getBucketPolicy( bucketId: string ): Promise<{ policyJson: string, etag: string | null } | null> {
    const rows = await db
...
export async function putBucketPolicy( bucketId: string, policyJson: string ): Promise<string> {
...
export async function getBucketCors( bucketId: string ): Promise<BucketCorsRuleRecord[]> {
    const rows = await db
        .select( {
    ...
    return rows.map( ( row ) => ( {
        ...
        allowedHeaders: row.allowedHeadersJson ? ( JSON.parse( row.allowedHeadersJson ) as string[] ) : [],
        exposeHeaders: row.exposeHeadersJson ? ( JSON.parse( row.exposeHeadersJson ) as string[] ) : [],
        maxAgeSeconds: row.maxAgeSeconds,
    } ) )
}

export async function replaceBucketCors( bucketId: string, rules: BucketCorsRuleRecord[] ): Promise<void> {
    await db.delete( bucketCorsRule ).where( eq( bucketCorsRule.bucketId, bucketId ) )
    if ( rules.length === 0 ) {
        return
    }

    await db.insert( bucketCorsRule ).values(
...
export async function listMultipartUploads( bucket: BucketContext ): Promise<MultipartUploadListItem[]> {
    const rows = await db
        ...
        .from( uploadAttempt )
        .where( and( eq( uploadAttempt.bucketId, bucket.bucketId ), eq( uploadAttempt.userId, bucket.userId ), eq( uploadAttempt.status, "pending" ) ) )
...
export async function listMultipartParts( bucket: BucketContext, uploadId: string ): Promise<MultipartPartListItem[]> {
    const uploads = await db
        .select( { id: uploadAttempt.id } )
        .from( uploadAttempt )
        .where( and( eq( uploadAttempt.id, uploadId ), eq( uploadAttempt.bucketId, bucket.bucketId ), eq( uploadAttempt.userId, bucket.userId ) ) )
        .limit( 1 )
