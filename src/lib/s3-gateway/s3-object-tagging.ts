...
async function resolveFileId( bucket: BucketContext, objectKey: string ): Promise<string | null> {
...
export async function getObjectTags( bucket: BucketContext, objectKey: string ): Promise<ObjectTag[]> {
...
export async function replaceObjectTags( bucket: BucketContext, objectKey: string, tags: ObjectTag[] ): Promise<void> {
    const targetFileId = await resolveFileId( bucket, objectKey )
    if ( !targetFileId ) {
    ...
    await db.insert( fileTag ).values( tags.map( ( tag ) => ( { fileId: targetFileId, tagKey: tag.key, tagValue: tag.value } ) ) )
}

export async function deleteObjectTags( bucket: BucketContext, objectKey: string ): Promise<void> {
    const targetFileId = await resolveFileId( bucket, objectKey )
    if ( !targetFileId ) {
        return
    }
...
export function objectTaggingXml( tags: ObjectTag[] ): string {
    const tagXml = tags
        .map( ( tag ) => `<Tag><Key>${escapeXml( tag.key )}</Key><Value>${escapeXml( tag.value )}</Value></Tag>` )
        .join( "" )
...
export function parseObjectTaggingXml( xml: string ): ObjectTag[] {
    const matches = [...xml.matchAll( /<Tag>\s*<Key>\s*([^<]*)\s*<\/Key>\s*<Value>\s*([^<]*)\s*<\/Value>\s*<\/Tag>/gi )]
