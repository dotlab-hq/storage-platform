...
export async function ensureBucketAcl(
  bucket: BucketContext,
): Promise<{ ownerCanonicalId: string; cannedAcl: CannedAcl }> {
  const existing = await db
    .select({
      ownerCanonicalId: bucketAcl.ownerCanonicalId,
      cannedAcl: bucketAcl.cannedAcl,
    })
    .from(bucketAcl)
  ...
  if (existing[0]) {
    return {
      ownerCanonicalId: existing[0].ownerCanonicalId,
      cannedAcl: existing[0].cannedAcl as CannedAcl,
    }
  }
...
export async function putBucketAcl(
  ...
  await db
    ...
    .onConflictDoUpdate({
      target: bucketAcl.bucketId,
      set: { cannedAcl, ownerCanonicalId },
    })
}

async function resolveObjectFileId(
  bucket: BucketContext,
  objectKey: string,
): Promise<string | null> {
...
export async function getObjectAclXml(
  bucket: BucketContext,
  objectKey: string,
): Promise<string> {
  ...
  const rows = await db
    .select({
      ownerCanonicalId: objectAcl.ownerCanonicalId,
      cannedAcl: objectAcl.cannedAcl,
    })
    .from(objectAcl)
...
export async function isBucketPublicReadable(
  bucketId: string,
): Promise<boolean> {
  const rows = await db
...
export async function isObjectPublicReadable(
  bucket: BucketContext,
  objectKey: string,
): Promise<boolean> {
  const fileId = await resolveObjectFileId(bucket, objectKey)
