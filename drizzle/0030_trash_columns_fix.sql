-- Migration: Add is_trashed and deletion_queued_at to folder table
-- Also adds missing updated_at column
...
-- Add missing updated_at column if not exists (use current timestamp as default)
ALTER TABLE folder ADD COLUMN updated_at integer NOT NULL DEFAULT (cast(strftime('%s', 'now') as integer));
